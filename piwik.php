 ...
 *
 * @author    Kenny Kang (winterrise@gmail.com)
 * @copyright Copyright 2018
 * @license   GNU Public License
 * @link      http://www.matomo.tw
 */
...
class PlgSystemPiwik extends JPlugin
{
	...
	public function addPiwik()
	{
		...
		// If the URL or the token is empty, do nothing
		if (empty($piwikUrl) || empty($tokenAuth))
		{
			return false;
		}
		...
		// Fix the IP-address
		$piwikTracker->setIp($this->getIpAddress());

		// Set an user variable
		if ($user->id > 0)
		{
			$piwikTracker->setCustomVariable(1, 'user_id', $user->id);
			$piwikTracker->setCustomVariable(2, 'user_email', $user->email);
			$piwikTracker->setCustomVariable(3, 'user_name', $user->name);
		}
		else
		{
			$piwikTracker->setCustomVariable(1, 'user_name', 'guest');
		}

		// Add an ecommerce-item if it exists
		$eitem = JFactory::getConfig()->get('piwik.ecommerce_item');
		if (!empty($eitem))
		{
			$piwikTracker->addEcommerceItem($eitem['sku'], $eitem['name'], $eitem['category'], $eitem['price'], $eitem['quantity']);
		}

		// Add a search if it exists
		$search = JFactory::getConfig()->get('piwik.search');
		if (!empty($search))
		{
			$piwikTracker->doTrackSiteSearch($search['keyword'], $search['category'], $search['num_results']);
		}

		// Add an action if it exists
		$action = JFactory::getConfig()->get('piwik.action');
		if (!empty($action))
		{
			$piwikTracker->doTrackAction($action['url'], $action['type']);
		}

		// Add a goal if it exists
		$goal = JFactory::getConfig()->get('piwik.goal');
		if (!empty($goal))
		{
			$piwikTracker->doTrackGoal($goal['id'], $goal['revenue']);
		}
	...
	public function isRawRequest()
	{
		...
		if ($application->input->getCmd('format') == 'raw')
		{
			return true;
		}
	...
	public function skipRobots()
	{
		$skipRobots = (bool) $this->params->get('skip_robots', 0);
